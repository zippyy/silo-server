...
Before promoting a normal user, Silo snapshots locally owned permissions and access-group assignment on the capability-scoped external identity. Repeated admin logins do not overwrite that snapshot, and the snapshot wins over authorization fields edited while the account is managed as an administrator. Demotion restores it and revokes native auth sessions in the same database transaction, then revokes Silo-owned Jellyfin and Audiobookshelf compatibility sessions after the commit. A persisted NULL permissions snapshot restores an empty permission set, while an intentionally ungrouped snapshot restores `access_group_id = NULL`. If a snapshotted group is later deleted, Silo uses the current default group when one exists. If a legacy managed administrator has no snapshot, Silo uses current normal-user default permissions and the current default access group; if no default group exists, the fallback is default normal-user permissions with `access_group_id = NULL` (ungrouped, not a library lockout).
